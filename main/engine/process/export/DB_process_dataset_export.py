...
from ..features.DB_process_hysteresis import compute_hysteresis_features, flatten_hysteresis_to_src
from ..features.DB_process_msbc_features import build_msbc_feature_payload, flatten_msbc_to_src
from ..features.DB_process_gcs_features import build_gcs_feature_payload, flatten_gcs_to_src
from ..features.DB_process_csd_dcsd_features import build_csd_dcsd_feature_payload, flatten_csd_dcsd_to_src
from ..features.DB_process_gbc_features import build_gbc_feature_payload, flatten_gbc_to_src
...
def build_method_v21_src_row(
    ...
    # Optional meta/context. Fill from timing/catalog if available.
    ...
    # ------------------------------------------------------------------
    # 3) structured feature payloads
    # ------------------------------------------------------------------
    decision_dt = timing.get("decision_dt") or timing.get("decision_ts")
    hyst_obj = compute_hysteresis_features(
        per_sigma_hist=per_sigma_hist or {},
        decision_dt=decision_dt,
        lookback_seconds=((config or {}).get("hyst", {}) or {}).get("lookback_seconds", 3600),
        tail_seconds=((config or {}).get("hyst", {}) or {}).get("tail_seconds", 120),
        align_tol_seconds=((config or {}).get("hyst", {}) or {}).get("align_tol_seconds", 3.0),
    )
    ...
    # ------------------------------------------------------------------
    # 4) flatten canonical src_* columns
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # 5) extra meta/context from calc_out if available
    # ------------------------------------------------------------------
