...
def build_regime_metrics(df: pd.DataFrame, idx: int, truth_col: str, base_col: str) -> dict:
    if idx < LOOKBACK:
        return {}
    ...
    strong_disagreement_density = float(((window["phase_disagree_count_3"] >= 2) | (window["fs_disagree_count_3"] >= 2) | (window["fast_alt_3epoch"] == 1)).mean())
    ...
    return {
        ...
        "strong_disagreement_density": strong_disagreement_density,
        ...
        "winner_triggers": stats_win["triggers"],
        "fallback_good_rate": stats_soft["good_rate"],
        "fallback_bad_rate": stats_soft["bad_rate"],
        "fallback_triggers": stats_soft["triggers"],
        "corea_good_rate": stats_corea["good_rate"],
        "corea_bad_rate": stats_corea["bad_rate"],
        "corea_triggers": stats_corea["triggers"],
        "tight_good_rate": stats_tight["good_rate"],
        "tight_bad_rate": stats_tight["bad_rate"],
        "tight_triggers": stats_tight["triggers"],
    }


def choose_regime_mode(metrics: dict) -> tuple[str, str]:
    if not metrics:
        return "OFF", "warmup_lt_30"
    quiet = metrics["wrong_bear_rate"] < 0.12 and metrics["disagreement_density"] < 0.45 and metrics["transition_density"] < 0.40
    aggressive = (
        metrics["wrong_bear_rate"] >= 0.24
        and metrics["disagreement_density"] >= 0.52
        and metrics["winner_bad_rate"] <= 0.34
        and (metrics["winner_good_rate"] >= 0.60 or metrics["winner_triggers"] == 0)
    )
    balanced = (
        metrics["wrong_bear_rate"] >= 0.18
        and metrics["disagreement_density"] >= 0.45
        and metrics["fallback_bad_rate"] <= 0.25
        and (metrics["fallback_good_rate"] >= 0.70 or metrics["fallback_triggers"] == 0)
    )
    safe = (
        metrics["wrong_bear_rate"] >= 0.14
        and (metrics["strong_disagreement_density"] >= 0.18 or metrics["mean_fs"] >= 0.9)
        and metrics["corea_bad_rate"] <= 0.10
    )

    if quiet:
        return "OFF", "quiet_regime"
    if aggressive:
        return "AGGRESSIVE", "regime_aggressive"
    if balanced:
        return "BALANCED", "regime_balanced"
    if safe:
        return "SAFE", "regime_safe"
    return "OFF", "regime_off_default"


def mode_to_policy(current: pd.Series, mode: str) -> tuple[str, str]:
    base_pred = current["pred_v22_base"]
    if base_pred != "Bear":
        return "base", f"base_pred={base_pred}"
    if int(current["ruleb_raw_warning"]) == 0:
        return "base", "ruleb_off"

    if mode == "AGGRESSIVE":
        if int(current["pm1_hinge_018"]) == 1:
            return "winner", "mode=AGGRESSIVE|current_h018"
        if int(current["pm1_hinge_022"]) == 1:
            return "winner_tight", "mode=AGGRESSIVE|current_h022"
        if int(current["pm1_centered_soft"]) == 1:
            return "fallback", "mode=AGGRESSIVE|current_soft"
        if int(current["osc_core_a"]) == 1:
            return "corea", "mode=AGGRESSIVE|current_corea"
        return "base", "mode=AGGRESSIVE|no_current_edge"

    if mode == "BALANCED":
        if int(current["pm1_centered_soft"]) == 1:
            return "fallback", "mode=BALANCED|current_soft"
        if int(current["pm1_hinge_018"]) == 1:
            return "winner", "mode=BALANCED|current_h018"
        if int(current["osc_core_a"]) == 1:
            return "corea", "mode=BALANCED|current_corea"
        return "base", "mode=BALANCED|no_current_edge"

    if mode == "SAFE":
        if int(current["osc_core_a"]) == 1:
            return "corea", "mode=SAFE|current_corea"
        if int(current["pm1_centered_soft"]) == 1 and abs(float(current["hinge_torsion"])) >= SOFT_HINGE:
            return "fallback", "mode=SAFE|current_soft"
        return "base", "mode=SAFE|no_current_edge"

    return "base", "mode=OFF|base"
...
def apply_adaptive_controller(df: pd.DataFrame, truth_col: str, base_col: str) -> pd.DataFrame:
    ...
    for i in range(len(df)):
        metrics = build_regime_metrics(df, i, truth_col, base_col)
        proposed_mode, regime_reason = choose_regime_mode(metrics)
        current = df.iloc[i]

        if i < LOOKBACK:
            mode = "OFF"
            final_reason = "warmup_lt_30"
        else:
            if hold_count > 0 and proposed_mode != hold_mode:
                mode = hold_mode
                hold_count -= 1
                final_reason = f"persist_{hold_mode}|{regime_reason}"
            else:
                mode = proposed_mode
                if mode in {"AGGRESSIVE", "BALANCED", "SAFE"}:
                    hold_mode = mode
                    hold_count = 2
                else:
                    hold_mode = mode
                    hold_count = 0
                final_reason = regime_reason

        policy, row_reason = mode_to_policy(current, mode)
        ...
        chosen_reason.append(f"{final_reason}|{row_reason}")
        adaptive_pred.append(pred)
        adaptive_trigger.append(trig)
        regime_rows.append(metrics)

    out = pd.DataFrame(index=df.index)
    out["adaptive_mode"] = pd.Series(chosen_mode, index=df.index, dtype="object")
    ...
    out["pred_adaptive_v2"] = pd.Series(adaptive_pred, index=df.index, dtype="object")
    out["trigger_adaptive_v2"] = pd.Series(adaptive_trigger, index=df.index, dtype="int64")
    override = pd.Series("NO_EFFECT", index=df.index, dtype="object")
    good = (out["trigger_adaptive_v2"] == 1) & (df[base_col] != df[truth_col])
    bad = (out["trigger_adaptive_v2"] == 1) & (df[base_col] == df[truth_col])
    override.loc[good] = "GOOD_NEUTRALIZE_WRONG_BASE"
    override.loc[bad] = "BAD_NEUTRALIZE_RIGHT_BASE"
    out["override_help_class_adaptive_v2"] = override
...
def summarize_trigger_quality(df: pd.DataFrame, policy_name: str) -> dict:
    trig_col = f"trigger_{policy_name}"
    help_col = f"override_help_class_{policy_name}"
    direction_mask = df[f"pred_{policy_name}"].isin(["Bull", "Bear"]) if policy_name != "adaptive_v2" else df["pred_adaptive_v2"].isin(["Bull", "Bear"])
...
def build_policy_usage_summary(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for policy, group in df.groupby("adaptive_policy", dropna=False):
        rows.append({
            ...
            "adaptive_triggers": int(group["trigger_adaptive_v2"].sum()),
            "good": int((group["override_help_class_adaptive_v2"] == "GOOD_NEUTRALIZE_WRONG_BASE").sum()),
            "bad": int((group["override_help_class_adaptive_v2"] == "BAD_NEUTRALIZE_RIGHT_BASE").sum()),
...
def build_chunk_stability(df: pd.DataFrame, truth_col: str) -> pd.DataFrame:
    ...
    model_map = {
        ...
        "adaptive_v2": "pred_adaptive_v2",
    }
    trig_map = {
        "base": None,
        "winner": "trigger_bear_pm1_h018",
        "fallback": "trigger_bear_pm1_soft",
        "adaptive_v2": "trigger_adaptive_v2",
    }
...
def build_overlap_report(df: pd.DataFrame) -> pd.DataFrame:
    ...
    adaptive = df["trigger_adaptive_v2"] == 1
    groups = {
...
def build_event_windows(df: pd.DataFrame, epoch_col: str) -> pd.DataFrame:
    centers = []
    centers.extend([(idx, "ADAPTIVE_TRIGGER") for idx in df.index[df["trigger_adaptive_v2"] == 1][:6]])
    remaining_mask = (
        (df["pred_v22_base"] == "Bear")
        & (df["truth"] == "Bull")
        & (df["ruleb_raw_warning"] == 1)
    ...
    for center_idx, label in centers:
        ...
        for rel in range(-WINDOW_RADIUS, WINDOW_RADIUS + 1):
            ...
            rows.append({
                ...
                "pred": row["pred_adaptive_v2"],
                "adaptive_policy": row["adaptive_policy"],
                "trig": int(row["trigger_adaptive_v2"]),
...
def main() -> None:
    ...
    model_map = {
        ...
        "Adaptive v275: Regime-mode 30-epoch controller": "pred_adaptive_v2",
    }
    ...
    rollout_rows = []
    order = [
        ("adaptive_v2", "Adaptive v275: Regime-mode 30-epoch controller"),
    ...
    print("\n=== V275 ROLLING ADAPTIVE LEADERBOARD ===")
    ...
    trig_quality = pd.DataFrame([
        ...
        summarize_trigger_quality(df, "adaptive_v2"),
    ])
    print("\n=== V275 TRIGGER QUALITY SUMMARY ===")
    ...
    decision_audit = df[[
        "epoch", "truth", "pred_v22_base", "pred_bear_core_a", "pred_bear_pm1_soft", "pred_bear_pm1_h018", "pred_adaptive_v2",
        "trigger_bear_core_a", "trigger_bear_pm1_soft", "trigger_bear_pm1_h018", "trigger_adaptive_v2",
        "override_help_class_bear_core_a", "override_help_class_bear_pm1_soft", "override_help_class_bear_pm1_h018",
        "override_help_class_adaptive_v2", "adaptive_policy", "adaptive_reason",
    ...
    decision_path = CSV_PATH.with_name("offline_v275_decision_audit.csv")
    trig_quality_path = CSV_PATH.with_name("offline_v275_trigger_quality_summary.csv")
    rollout_path = CSV_PATH.with_name("offline_v275_rollout_summary.csv")
    usage_path = CSV_PATH.with_name("offline_v275_policy_usage_summary.csv")
    reason_path = CSV_PATH.with_name("offline_v275_reason_summary.csv")
    chunk_path = CSV_PATH.with_name("offline_v275_chunk_stability.csv")
    overlap_path = CSV_PATH.with_name("offline_v275_overlap_report.csv")
    remain_path = CSV_PATH.with_name("offline_v275_remaining_false_negative_audit.csv")
    windows_path = CSV_PATH.with_name("offline_v275_event_windows.csv")
    model_path = CSV_PATH.with_name("offline_v275_model_comparison.csv")
