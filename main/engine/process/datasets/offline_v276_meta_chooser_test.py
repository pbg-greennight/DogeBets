...
def main() -> None:
    ...
    specs = [
        ...
        ('Adaptive v276: KNN meta-chooser (30-epoch lookback)', 'pred_adaptive_meta'),
    ]
    for label, pred_col in specs:
    ...
    df.to_csv(out_dir / 'gaussian_fan_dataset_v2_2_with_v276_meta_diagnostics.csv', index=False)
    df.to_csv(out_dir / 'offline_v276_decision_audit.csv', index=False)
    trigger_df.to_csv(out_dir / 'offline_v276_trigger_quality_summary.csv', index=False)
    summary_df.to_csv(out_dir / 'offline_v276_rollout_summary.csv', index=False)
    policy_usage.to_csv(out_dir / 'offline_v276_policy_usage_summary.csv', index=False)
    reason_usage.to_csv(out_dir / 'offline_v276_reason_summary.csv', index=False)
    chunk_df.to_csv(out_dir / 'offline_v276_chunk_stability.csv', index=False)
    overlap_df.to_csv(out_dir / 'offline_v276_overlap_report.csv', index=False)
    remaining_fn.to_csv(out_dir / 'offline_v276_remaining_false_negative_audit.csv', index=False)
    summary_df.to_csv(out_dir / 'offline_v276_model_comparison.csv', index=False)

    print(f"\nSaved dataset: {out_dir / 'gaussian_fan_dataset_v2_2_with_v276_meta_diagnostics.csv'}")
    print(f"Saved decision audit: {out_dir / 'offline_v276_decision_audit.csv'}")
    print(f"Saved trigger quality summary: {out_dir / 'offline_v276_trigger_quality_summary.csv'}")
    print(f"Saved rollout summary: {out_dir / 'offline_v276_rollout_summary.csv'}")
    print(f"Saved policy usage summary: {out_dir / 'offline_v276_policy_usage_summary.csv'}")
    print(f"Saved reason summary: {out_dir / 'offline_v276_reason_summary.csv'}")
    print(f"Saved chunk stability: {out_dir / 'offline_v276_chunk_stability.csv'}")
    print(f"Saved overlap report: {out_dir / 'offline_v276_overlap_report.csv'}")
    print(f"Saved remaining false negative audit: {out_dir / 'offline_v276_remaining_false_negative_audit.csv'}")
    print(f"Saved model comparison: {out_dir / 'offline_v276_model_comparison.csv'}")
