...
def build_locked_predictions(df: pd.DataFrame, base_col: str, truth_col: str) -> pd.DataFrame:
    ...
    bear_ruleb = (df[base_col] == "Bear") & (df["ruleb_raw_warning"] == 1)
    masks = {
...
def action_prediction(row: pd.Series, action: str) -> str:
    if action == "base":
        return row["pred_v22_base"]
    return row[f"pred_{action}"]


def action_trigger(row: pd.Series, action: str) -> int:
    if action == "base":
        return 0
    return int(row[f"trigger_{action}"])
...
def fit_and_predict_action(hist: pd.DataFrame, current: pd.Series, lookback: int) -> tuple[str, str]:
    eligible_hist = hist[(hist["pred_v22_base"] == "Bear") & (hist["ruleb_raw_warning"] == 1)].copy()
...
def trigger_quality(df: pd.DataFrame, pred_col: str) -> dict:
    trigger = (df[pred_col] == "Neutral") & (df["pred_v22_base"] == "Bear")
    base_wrong = (df["pred_v22_base"] == "Bear") & (df["truth"] != "Bear")
    base_right = (df["pred_v22_base"] == "Bear") & (df["truth"] == "Bear")
    return {
...
def capture_wrong_bear(df: pd.DataFrame, pred_col: str) -> float:
    universe = ((df["pred_v22_base"] == "Bear") & (df["ruleb_raw_warning"] == 1) & (df["truth"] == "Bull"))
    if int(universe.sum()) == 0:
        return 0.0
    capture = ((df[pred_col] == "Neutral") & universe).sum()
...
def main() -> None:
    ...
    if base_col != "pred_v22_base":
        df["pred_v22_base"] = df[base_col].copy()
    ...
    for lb in LOOKBACK_WINDOWS:
        ...
        for pol, g in grp:
            policy_rows.append({
                ...
                "triggers": int(((g[f'pred_meta_{lb}'] == 'Neutral') & (g['pred_v22_base'] == 'Bear')).sum()),
                "good": int((((g[f'pred_meta_{lb}'] == 'Neutral') & (g['pred_v22_base'] == 'Bear') & (g['truth'] == 'Bull'))).sum()),
                "bad": int((((g[f'pred_meta_{lb}'] == 'Neutral') & (g['pred_v22_base'] == 'Bear') & (g['truth'] == 'Bear'))).sum()),
            })
            print(f"lookback={lb} | policy={pol} | rows={len(g)} | bear_rows={int((g['pred_v22_base'] == 'Bear').sum())} | triggers={int(((g[f'pred_meta_{lb}'] == 'Neutral') & (g['pred_v22_base'] == 'Bear')).sum())}")
    ...
    for lb in LOOKBACK_WINDOWS:
        ...
        for reason, count in top.items():
            ...
            print(f"lookback={lb} | reason={reason} | rows={len(g)} | triggers={int(((g[f'pred_meta_{lb}'] == 'Neutral') & (g['pred_v22_base'] == 'Bear')).sum())}")
