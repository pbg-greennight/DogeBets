...
def print_gaussian_bell_curve_series_dump(
    ...
    if not bool(config.get('LOG_BELL_CURVE_SERIES_DUMP', True)):
        return
    if not _section_on(config, "gbc_sd", True):
        return
    ...
    logger.info(f"Window: (per-sigma PV → NOW) | points(max)={max_points}")
    ...
    if not bool(config.get('LOG_BELL_CURVE_DIAGNOSTICS', True)):
        return
    if not _section_on(config, "gbc_diag", True):
        return
