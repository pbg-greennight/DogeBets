...
from main.engine.process.core.DB_process_catalog import FeatureCatalog
from main.engine.process.core.DB_process_config import cfg, tail_print_seconds
from main.engine.process.export.DB_process_csv import write_long_snapshot, write_wide_snapshot
from main.engine.process.core.DB_process_gauss_sources import fetch_plot_series, get_gauss_registry, start_gauss_sources_once
from main.engine.process.utils.DB_process_series_cache import SeriesCache
...
from main.engine.process.utils.DB_process_time import _fmt_ts, compute_windows, get_epoch_timing
from main.engine.process.core.DB_process_trend import calculate_trend
from main.engine.process.utils.DB_process_types import EpochTiming, Windows

from main.engine.process.DB_process_v21_snapshot_log import (
    build_v21_snapshot,
    write_v21_snapshot,
)
...
def _mapped_context_active(mapped: Dict[str, Any]) -> bool:
    slope_keys = {
        "slope_s8",
        "slope_s23",
        "slope_s38",
...
def _write_v21_snapshot_from_trend(
    timing: EpochTiming,
    decision_dt: datetime,
    trend_obj: Any,
) -> None:
    """
    Canonical v21 snapshot writer.

    Uses the same decision object that was just printed so the saved snapshot and
    the human-readable live decision stay aligned.
    """
    try:
        model = getattr(trend_obj, "model", None)
        if model != "v21_live":
            return

        extras = getattr(trend_obj, "extras", None) or {}
        extras = _as_dict(extras)

        inputs = _as_dict(extras.get("inputs"))
        latest_gauss = _as_dict(inputs.get("latest_gauss"))
        scores = _as_dict(extras.get("scores"))
        mapped = _as_dict(extras.get("mapped_features"))
        flags = _as_dict(extras.get("flags"))
        hyst_meta = _as_dict(extras.get("hyst_meta"))
        src_summary = _as_dict(extras.get("src_summary"))
        raw_td = _as_dict(extras.get("raw_td_features"))

        gauss_raw = _as_dict(raw_td.get("gauss"))
        hyst_raw = _as_dict(raw_td.get("hysteresis"))
        raw_slopes = _as_dict(gauss_raw.get("slopes"))

        snapshot = build_v21_snapshot(
            source_epoch=int(getattr(timing, "curr_epoch", 0) or 0),
            predicted_next_epoch=int(getattr(timing, "next_epoch", 0) or 0),
            decision_time=decision_dt,
            next_epoch_time=getattr(timing, "dt_next", None),
            trend=str(getattr(trend_obj, "trend", "Neutral")),
            confidence=float(getattr(trend_obj, "confidence", 0.0) or 0.0),
            model=str(model or ""),
            notes=str(getattr(trend_obj, "notes", "") or getattr(trend_obj, "reason", "") or ""),
            inputs={
                "s8": inputs.get("s8"),
                "s23": inputs.get("s23"),
                "s53": inputs.get("s53"),
                "s83": inputs.get("s83"),
                "fast_min": inputs.get("fast_min"),
                "reversal_min": inputs.get("reversal_min"),
                "mid_dom_ratio": inputs.get("mid_dom_ratio"),
                "decision_min": inputs.get("decision_min"),
            },
            gauss={
                "g8": latest_gauss.get("s8"),
                "g23": latest_gauss.get("s23"),
                "g38": latest_gauss.get("s38"),
                "g53": latest_gauss.get("s53"),
                "g68": latest_gauss.get("s68"),
                "g83": latest_gauss.get("s83"),
            },
            scores=scores,
            mapped=mapped,
            hyst_meta=hyst_meta,
            raw_keys={
                "gauss_keys": list(gauss_raw.keys()),
                "gauss_slope_keys": list(raw_slopes.keys()),
                "hyst_keys": list(hyst_raw.keys()),
            },
            raw_slopes={
                "s8": raw_slopes.get("s8"),
                "s23": raw_slopes.get("s23"),
                "s38": raw_slopes.get("s38"),
                "s53": raw_slopes.get("s53"),
                "s68": raw_slopes.get("s68"),
                "s83": raw_slopes.get("s83"),
            },
            feature_flags={
                "mapped_context_active": bool(flags.get("mapped_context_active", _mapped_context_active(mapped))),
                "src_row_available": bool(flags.get("src_row_available", bool(src_summary))),
                "fast_agree": flags.get("fast_agree"),
                "mid_dom": flags.get("mid_dom"),
                "mid_contradicts_fast": flags.get("mid_contradicts_fast"),
                "branch": flags.get("branch"),
            },
            src_summary=src_summary,
        )

        text_path, jsonl_path = write_v21_snapshot(snapshot)
        print(f"[v21_snapshot] wrote → text={text_path} | jsonl={jsonl_path}")
    except Exception as e:
        print(f"[v21_snapshot][ERROR] {e}")
...
def process_epoch_window(
    ...
    _write_v21_snapshot_from_trend(timing, decision_dt, trend)
