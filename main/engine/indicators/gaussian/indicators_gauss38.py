# main/engine/indicators/gaussian/indicators_gauss38.py
"""
IND_gauss_38 (VIS)

Centered (non-causal) Gaussian smoothing of BTC close using sigma=38 (in bars),
with edge-safe truncation + renormalization so the curve renders to the ends.

- Consumes data from indicators.py
- Maintains rolling plot series (ts, g38)
- Logs one row per new bar via indicators_log.log_gauss38_bar()
"""
...
_plot_state: Dict[str, List[Any]] = {"ts": [], "g38": []}

_last_ts_utc: Optional[str] = None

print("GAUSS38 sees indicators from:", getattr(indicators, "__file__", "<no file>"))
...
def get_plot_series() -> Dict[str, Any]:
    with _plot_lock:
        return {"ts": list(_plot_state["ts"]), "g38": list(_plot_state["g38"])}
...
def _loop():
    ...
    while True:
        ...
        g38_series = _gaussian_smooth_centered_edge_safe(close_list, _KERNEL)

        with _plot_lock:
            _plot_state["ts"] = list(ts_list)
            _plot_state["g38"] = list(g38_series)

        latest_g38 = g38_series[-1] if g38_series else None
        ...
        indicators_log.log_gauss38_bar(ts_utc, ochlv, epoch_snap, {"gauss_38": latest_g38})
