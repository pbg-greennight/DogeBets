# main/engine/indicators/gaussian/indicators_gauss8.py
"""
IND_gauss_8 (VIS)

Centered (non-causal) Gaussian smoothing of BTC close using sigma=8 (in bars),
with edge-safe truncation + renormalization so the curve renders to the ends.

- Consumes data from indicators.py
- Maintains rolling plot series (ts, g8)
- Logs one row per new bar via indicators_log.log_gauss8_bar()
"""
...
_plot_state: Dict[str, List[Any]] = {"ts": [], "g8": []}

_last_ts_utc: Optional[str] = None

print("GAUSS8 sees indicators from:", getattr(indicators, "__file__", "<no file>"))
...
def get_plot_series() -> Dict[str, Any]:
    with _plot_lock:
        return {"ts": list(_plot_state["ts"]), "g8": list(_plot_state["g8"])}
...
def _loop():
    ...
    while True:
        ...
        g8_series = _gaussian_smooth_centered_edge_safe(close_list, _KERNEL)

        with _plot_lock:
            _plot_state["ts"] = list(ts_list)
            _plot_state["g8"] = list(g8_series)

        latest_g8 = g8_series[-1] if g8_series else None
        ...
        indicators_log.log_gauss8_bar(ts_utc, ochlv, epoch_snap, {"gauss_8": latest_g8})
