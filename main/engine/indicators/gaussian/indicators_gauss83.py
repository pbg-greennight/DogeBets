# main/engine/indicators/gaussian/indicators_gauss83.py
"""
IND_gauss_83 (VIS)

Centered (non-causal) Gaussian smoothing of BTC close using sigma=83 (in bars),
with edge-safe truncation + renormalization so the curve renders to the ends.

- Consumes data from indicators.py
- Maintains rolling plot series (ts, g83)
- Logs one row per new bar via indicators_log.log_gauss83_bar()
"""
...
_plot_state: Dict[str, List[Any]] = {"ts": [], "g83": []}

_last_ts_utc: Optional[str] = None

print("GAUSS83 sees indicators from:", getattr(indicators, "__file__", "<no file>"))
...
def get_plot_series() -> Dict[str, Any]:
    with _plot_lock:
        return {"ts": list(_plot_state["ts"]), "g83": list(_plot_state["g83"])}
...
def _loop():
    ...
    while True:
        ...
        g83_series = _gaussian_smooth_centered_edge_safe(close_list, _KERNEL)

        with _plot_lock:
            _plot_state["ts"] = list(ts_list)
            _plot_state["g83"] = list(g83_series)

        latest_g83 = g83_series[-1] if g83_series else None
        ...
        indicators_log.log_gauss83_bar(ts_utc, ochlv, epoch_snap, {"gauss_83": latest_g83})
