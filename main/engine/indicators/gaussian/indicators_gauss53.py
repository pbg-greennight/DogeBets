# main/engine/indicators/gaussian/indicators_gauss53.py
"""
IND_gauss_53 (VIS)

Centered (non-causal) Gaussian smoothing of BTC close using sigma=53 (in bars),
with edge-safe truncation + renormalization so the curve renders to the ends.

- Consumes data from indicators.py
- Maintains rolling plot series (ts, g53)
- Logs one row per new bar via indicators_log.log_gauss53_bar()
"""
...
_plot_state: Dict[str, List[Any]] = {"ts": [], "g53": []}

_last_ts_utc: Optional[str] = None

print("GAUSS53 sees indicators from:", getattr(indicators, "__file__", "<no file>"))
...
def get_plot_series() -> Dict[str, Any]:
    with _plot_lock:
        return {"ts": list(_plot_state["ts"]), "g53": list(_plot_state["g53"])}
...
def _loop():
    ...
    while True:
        ...
        g53_series = _gaussian_smooth_centered_edge_safe(close_list, _KERNEL)

        with _plot_lock:
            _plot_state["ts"] = list(ts_list)
            _plot_state["g53"] = list(g53_series)

        latest_g53 = g53_series[-1] if g53_series else None
        ...
        indicators_log.log_gauss53_bar(ts_utc, ochlv, epoch_snap, {"gauss_53": latest_g53})
